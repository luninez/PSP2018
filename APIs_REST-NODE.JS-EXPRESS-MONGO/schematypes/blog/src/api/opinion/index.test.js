...
test('GET /opiniones/:id 200', async () => {
  const { status, body } = await request(app())
...
test('GET /opiniones/:id 404', async () => {
  const { status } = await request(app())
    .get(apiRoot + '/123456789098765432123456')
  expect(status).toBe(404)
})

test('PUT /opiniones/:id 200', async () => {
  const { status, body } = await request(app())
...
test('PUT /opiniones/:id 404', async () => {
  const { status } = await request(app())
...
test('DELETE /opiniones/:id 204', async () => {
  const { status } = await request(app())
    .delete(`${apiRoot}/${opinion.id}`)
  expect(status).toBe(204)
})

test('DELETE /opiniones/:id 404', async () => {
  const { status } = await request(app())
